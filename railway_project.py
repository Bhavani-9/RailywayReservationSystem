...
class train():
    def __init__(self,tname='',tnum=0,arr_time='',dep_time='',src='',des='',day_of_travel='',seat_available_in_1AC=0,seat_available_in_2AC=0,seat_available_in_3AC=0,seat_available_in_SL=0,fare_1AC=0,fare_2AC=0,fare_SL=0):
        self.tname=tname
        self.tnum=tnum
    ...
    def check_availability(self,coach='',ticket_num=0):
        ...
        if coach not in ('SL','1AC','2AC'):
            ...
            coach=input('Enter your coach(SL/1AC/2AC):')
        else:
            if self.seats[coach] == 0:
                return False
            elif self.seats[coach] >= ticket_num:       # doubt
                return True
            else:
                return False
    def book_ticket(self,coach='',no_of_tickets=0):
        self.seats[coach] = self.seats[coach]-no_of_tickets
        return True
    

class ticket:
    def __init__(self,train,user,ticket_num,coach):
        self.pnr=str(train.tnum)+str(user.uid)+str(random.randint(100,999))
        self.train_num=train.tnum
        self.coach=coach
        self.uid=user.uid
        self.train_name=train.tname
        self.user_name=user.name
        self.train_num=ticket_num
...
class acceptors:
    def accept_uid():
        ...
        try:
            ...
            print("Please enter your user ID properly.")
...
def check_seat_availability(flag=' '):
    ...
    for i in trains:
        if trains[i].src == src and trains[i].des == des:
            print("Train Name : ",trains[i].tname," ","Number",trains[i].tnum," ","Day of Travel : ",trains[i].day_of_travel)
            flag_2 +=1
        if flag_2 == 0:
...
def check_pnr(pnr =''):
    if pnr == '':
        ...
        print("User name : ",ticket_dict[pnr].user_name)
        print("Train name : ",ticket_dict[pnr].train_name)
        print("Train number : ",ticket_dict[pnr].train_num,"Source : ",trains[ticket_dict[pnr].tain_num].src,"Destination : ",trains[ticket_dict[pnr].train_num].des)
        print("No. of tickets booked : ",ticket_dict[pnr].ticket_num)
        print()
...
trains = {t1.tnum:t1,t2.tnum:t2,t3.tnum:t3}
u1 = user(1111,'Hiral','cuttack','9191919191','Hiral')
u2 = user(2322,'John',"Vizag",'8348784828','AKP')
